...
with open('quiverquant/House/housetrading_live.json', 'w') as file_object:
...
for ticker in file1.readlines():
    ticker = ticker.strip()
    all_data = f"https://api.quiverquant.com/beta/historical/housetrading/{ticker}"
    headers = {
    ...
    try:
        ...
        trading = r.json()
        if trading:
            with open('quiverquant/House/housetrading_historical/'+ticker+'.json', 'w') as file_object:
