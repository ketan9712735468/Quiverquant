...
for ticker in file1.readlines():
    ...
    if count%80 == 0:
        time.sleep(8)
    all_data = f"https://api.quiverquant.com/beta/historical/offexchange/{ticker}"
    headers = {
    ...
    try:
        ...
        trading = r.json()
        if trading:
            with open('quiverquant/Off-exchange/offexchange_historical/'+ticker+'.json', 'w') as file_object:
