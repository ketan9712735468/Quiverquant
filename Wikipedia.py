...
with open('quiverquant/Wikipedia Pageviews/wikipedia_live.json', 'w') as file_object:
...
for ticker in file1.readlines():
    ticker = ticker.strip()
    all_data = f"https://api.quiverquant.com/beta/historical/wikipedia/{ticker}"
    headers = {
    ...
    try:
        ...
        trading = r.json()
        if trading:
            with open('quiverquant/Wikipedia Pageviews/wikipedia_historical/'+ticker+'.json', 'w') as file_object:
