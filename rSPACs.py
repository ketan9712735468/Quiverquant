...
# with open('quiverquant/rSPACs/spacs_live.json', 'w') as file_object:
...
for ticker in file1.readlines():
    ...
    if count%80 == 0:
        time.sleep(8)
    all_data = f"https://api.quiverquant.com/beta/historical/spacs/{ticker}"
    headers = {
    ...
    try:
        ...
        trading = r.json()
        if trading:
            with open('quiverquant/rSPACs/spacs_historical/'+ticker+'.json', 'w') as file_object:
